...
void GameFramework::CreateGameFramework(HWND &Hwnd)
{
	...
	CreateResource();
	// 6. Direct2D, DirectWrite, D3D11on12를 사용한 텍스트 출력
...
void GameFramework::CreateDirectDevice()
{
	...
	// 3. 사용이 끝난 COM 객체는 반드시 Release 해야 합니다.
...
void GameFramework::CreateSwapChain()
{
	...
	SwapChainDesc.BufferCount = SWAP_CHAIN_COUNT; // 스왑 체인의 버퍼 개수, 전면 버퍼와 후면 버퍼를 사용
...
void GameFramework::CreateResource()
{
	...
	DescriptorHeapDesc.NumDescriptors = SWAP_CHAIN_COUNT; // 서술자의 개수 결정 - 전면과 후면을 사용할 것이므로 2
...
void GameFramework::GameFrameworkLoop()
{
	...
	// 3D 모델 객체와 2D 텍스트 객체의 애니메이션 및 렌더링 작업이 끝났으므로 프리젠트 수행
