...
void Camera::CreateCamera(ID3D12Device* Device, ID3D12GraphicsCommandList* CommandList, Player* Target)
{
...
void Camera::Update(float ElapsedTime, DirectX::XMFLOAT3 LookAt)
{
	...
	Right = m_Target->GetCameraRight(), Up = m_Target->GetCameraUp(), Look = m_Target->GetCameraLook();
	...
	DirectX::XMStoreFloat3(&Position, DirectX::XMVectorAdd(DirectX::XMLoadFloat3(&m_Target->GetPosition()), DirectX::XMLoadFloat3(&Offset)));
	...
	if (Distance > 0.f) {
		...
		DirectX::XMStoreFloat4x4(&RotateInfo, DirectX::XMMatrixLookAtLH(DirectX::XMLoadFloat3(&m_Position), DirectX::XMLoadFloat3(&LookAt), DirectX::XMLoadFloat3(&m_Target->GetCameraUp())));
	...
	DirectX::XMStoreFloat3(&m_Right, DirectX::XMVector3Normalize(DirectX::XMVector3Cross(DirectX::XMLoadFloat3(&m_Up), DirectX::XMLoadFloat3(&m_Look))));
	DirectX::XMStoreFloat3(&m_Up, DirectX::XMVector3Normalize(DirectX::XMVector3Cross(DirectX::XMLoadFloat3(&m_Look), DirectX::XMLoadFloat3(&m_Right))));
